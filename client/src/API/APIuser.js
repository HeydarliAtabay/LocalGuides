...
async function sendTripRequest(StartDate, EndDate,Guide,City,TouristCount,Status,user) {
    ...
    async function acceptTripRequest(user, guide) {
        ...
        async function filterRequest(price, rating) {
            return new Promise((resolve, reject) => {
                fetch('/api/filterRequest/', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
                    body: JSON.stringify({price: price, rating: rating})
                }).then((response) => {
                    if (response.ok) {
                        resolve(response.json());
                    } else {
...
const userMethods = {userLogin, filterRequest, acceptTripRequest, sendTripRequest, uploadPhoto, getMyTrips, editProfile, getChats, getSingleChat, getGuide, getGuideList, addRating};
